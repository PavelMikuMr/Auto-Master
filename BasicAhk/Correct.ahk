...
; c1 = игнорировать введенный регистр, всегда использовать один и тот же регистр для вывода
; * = немедленное изменение (нет необходимости в пробеле, точке или вводе)
; ? = активируется, даже если символ, введенный непосредственно перед ним, является буквенно-цифровым
; r = необработанный вывод

;------------------------------------------------------------------------------
; СПИСОК ИЗМЕНЕНИЙ:
;
; 2011-03-21 и после: См readme.md.
;
; Sep 13 2007: Добавлено больше орфографических ошибок.
; Добавлено исправление для -ign -> -ing, который игнорирует такие слова, как "sign".
; Добавлены разделы начала/окончания слов, чтобы охватить больше параметров.
; Добавлен раздел автоакценций для таких слов, как невеста, наивность и т.д.
; 28 февраля 2007: Добавлены другие распространенные орфографические ошибки на основе автозамены MS Word.
; Добавлена дополнительная автокоррекция 2 последовательных заглавных букв.
; 24 сентября 2006: Первый релиз Джима Бьянколо (http://www.biancolo.com)
;
; ЗНАКОМСТВО
;
; Это сценарий AutoHotKey, который реализует автозамену для нескольких
; "Списки распространенных орфографических ошибок":
;
; Это не заменяет правильную проверку орфографии, такую как в Firefox, Word и т. Д.
; Обычно лучше, чтобы неопределенные опечатки были выделены средством проверки орфографии.
; чем «исправить» их неправильно, чтобы они больше даже не были пойманы
; проверка орфографии: это не работа автокорректора, чтобы исправить *все*
; орфографические ошибки, но только те, которые очень явно неправильны.
;
; По предложению вы можете добавить свои собственные исправления к любому
; выделенное слово нажатием Win+A. Они будут добавлены в отдельный файл,
; чтобы можно было безопасно обновить этот файл без перезаписи изменений.
;
; Некоторые записи имеют более одного возможного разрешения (achive->achieve/archive)
; или явно являются вопросом преднамеренного личного стиля письма (хочу, цвет)
;
; Они были помещены в конце этого файла и закомментированы, чтобы вы могли
; легко редактируйте и добавляйте их обратно, как вам нравится, с учетом ваших предпочтений.
;
; ИСТОЧНИКОВ
;
; http://en.wikipedia.org/wiki/Wikipedia:Lists_of_common_misspellings
; http://en.wikipedia.org/wiki/Wikipedia:Typo
; Список автозамен Microsoft Office
; Автор сценария: jaco0646 http://www.autohotkey.com/forum/topic8057.html
; Список автозамены OpenOffice
; Пресс-релиз TextTrust
; Предложения пользователей.
;
; СОДЕРЖАНИЕ
;
; Параметры
; AUto-COrrect TWo COnsecutive CApitals (закомментирован по умолчанию)
; Код Win+H
; Исправление для -ign вместо -ing
; Окончания слов
; Начало слов
; Английские слова с акцентом
; Распространенные орфографические ошибки - основной список
; Неоднозначные записи - закомментированы
;------------------------------------------------------------------------------
...
Menu, Tray, Icon, %A_MyDocuments%\AutoHotkey\Ico\AhkKeys\Correct.ico
